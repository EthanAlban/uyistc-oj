...
func init() {
	...
	//beego 注册模型
	orm.RegisterModel(new(User), new(Annnocement), new(ProblemType), new(Problems), new(Tags), new(ProblemTags), new(SysInfo), new(Judger), new(Submission), new(Language), new(Logs), new(Contests))
	O = orm.NewOrm()
}
