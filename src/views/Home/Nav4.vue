<template>
  <div>
    ...
    <div v-if="$root.user_profile !== null">
      <el-table :data="submissons_list" style="width: 100%" max-height="800">
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label>
                <el-button type="primary" @click="getCode(props.row.id)" plain>查看代码</el-button>
              </el-form-item>
              <el-form-item label="错误提示">
                <span style="color: red">
                  {{
                  props.row.statistic_info.err_info
                  }}
                </span>
              </el-form-item>
            </el-form>
        ...
        <el-table-column label="问题编号" prop="problem">
          <template slot-scope="scope">
            <el-button
              @click="problemDetail(scope.row.problem)"
              type="text"
              size="small"
            >{{scope.row.problem}}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="得分" prop="statistic_info.score"></el-table-column>
...
export default {
  mounted() {
    this.GetSubmissions();
    if (this.$root.user_profile === null) {
      const h = this.$createElement;
      this.$notify({
        title: "未登录",
        message: h("i", { style: "color: teal" }, "未登录无法查看提交记录")
      });
    }
  },
  data() {
  ...
  methods: {
    ...
    GetSubmissions(page, limit, offset) {
      ...
      this.$axios
        .submissions_myself(this.page, this.limit, this.offset)
        .then(res => {
          // 将编译结果和编码对应
          this.submissons_list = res.data.results;
    ...
    // 进入问题详情页
    problemDetail(id) {
      this.$axios.problem_detail_by_id(id).then(res => {
        this.selfLog(res);
        this.$router.push(`/problemDetail/${res.data._id}`);
      });
    },
    // 提交列表查看提交的代码
    getCode(submission_id) {
      this.$axios.submission_id(submission_id).then(res => {
        this.$confirm(res.data.code, "提交代码", {
          distinguishCancelAndClose: true,
          confirmButtonText: "保存到剪贴板",
          cancelButtonText: "取消"
        })
          .then(() => {
            this.$copyText(res.data.code);
            this.$message({
              type: "info",
              message: "已复制"
            });
