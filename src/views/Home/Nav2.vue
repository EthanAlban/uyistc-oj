<template>
	<div>
		...
		<el-drawer :visible.sync="tag_drawer" append-to-body direction="btt" :with-header="false">
			<el-tag v-for="(tag,idx) in tags" :key="idx" :disable-transitions="false" :type="tag_types[idx%5]"
				effect="dark" style="position:flex;float:left;margin-left:10px;margin-top:10px"
				@click="choose_tag_problem(tag)">{{tag.TagName}}
			</el-tag>
		</el-drawer>
        ...
        <template slot-scope="scope">
          <el-tag v-for="(tag,idx) in scope.row.tags" :key="tag" size="mini" :type="tag_types[idx%5]" effect="dark"
			...
			<el-table-column prop="Level" label="难度" width="180">
				<template slot-scope="scope">
					<el-rate v-model="scope.row.Level" disabled show-score :max="max" text-color="#ff9900"
						score-template>
					</el-rate>
				</template>
			</el-table-column>
			...
			<el-table-column prop="pass_rate" label="通过率">
				<template slot-scope="scope">
					...
					<div v-else>
						<el-progress :text-inside="true" :stroke-width="26" :percentage="scope.row.pass_rate">
						</el-progress>
					</div>
				</template>
			</el-table-column>
		...
		<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="page"
			:page-sizes="[1,10, 20, 40]" :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="total"
...
<script>
	export default {
		...
		methods: {
			...
			// 获取问题列表
			GetProblems() {
				this.$problem_axios.ProblemListPage(this.limit, this.offset).then(res => {
					this.selfLog(res)
					if (res.errcode === 200) {
						this.total = res.data.total
						this.problems_list = res.data.problems
						for (let problem of this.problems_list) {
							problem['pass_rate'] = parseFloat(((problem.AcceptSubmissions / problem
								.TotalSubmissions) * 100).toFixed(2))
